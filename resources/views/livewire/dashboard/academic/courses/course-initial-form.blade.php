<div x-data="{ 
    open: false
     ...
     wire:ignore.self
     @open-modal.window="
         if ($event.detail[0] === 'course-initial-form') { 
             open = true; 
         }
     " 
     @close-modal.window="
    ...
    <div class="flex items-center justify-center min-h-screen p-4 text-center sm:p-0 pointer-events-none">
        <div x-show="open"
             ...
             class="pointer-events-auto relative bg-white dark:bg-surface-800 rounded-2xl text-start overflow-hidden shadow-2xl border border-surface-200 dark:border-surface-700 transform transition-all sm:my-8 sm:w-full sm:max-w-md">
            ...
            <form @submit.prevent="$wire.save()">
                ...
                <div class="p-6 space-y-6">
                    <p class="text-sm text-surface-500">أدخل اسم وكود الدورة للبدء، ستتمكن من تعديل كافة التفاصيل وبناء المحتوى في الخطوة التالية.</p>
                    
                    <div>
                        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-1">اسم الدورة <span class="text-red-500">*</span></label>
                        <input type="text" wire:model.blur="name" class="premium-input w-full" placeholder="مثال: دبلوم الأمن السيبراني المتقدم">
                        @error('name') <span class="text-sm text-red-500 mt-1 block">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-1">كود الدورة (Code) <span class="text-red-500">*</span></label>
                        <input type="text" wire:model.blur="code" class="premium-input w-full text-left" dir="ltr" placeholder="CS-2025-01">
