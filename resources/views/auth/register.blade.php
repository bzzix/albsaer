<x-guest-layout>
    ...
    <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-[520px] reveal-anim">
        ...
        <div class="glass-panel overflow-hidden rounded-3xl p-8 sm:p-12 shadow-glass relative">
            ...
            <form method="POST" action="{{ route('register') }}" class="space-y-5 relative z-10" x-data="{ showPw: false, showConfirmPw: false }">
                @csrf

                <!-- Name Input -->
                <div class="space-y-2 group">
                    <label for="name" class="block text-sm font-bold text-surface-700 transition-colors group-focus-within:text-primary-600">الاسم كاملًا</label>
                    <div class="relative">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <!-- Password Input -->
                    <div class="space-y-2 group">
                        <label for="password" class="block text-sm font-bold text-surface-700 transition-colors group-focus-within:text-primary-600">كلمة المرور</label>
                        <div class="relative">
                            <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-4">
                                <svg class="h-5 w-5 text-surface-400 group-focus-within:text-primary-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                ...
                @if (Laravel\Jetstream\Jetstream::hasTermsAndPrivacyPolicyFeature())
                    <div class="pt-2">
                        <label class="relative flex items-center cursor-pointer group">
                            <input id="terms" name="terms" type="checkbox" class="peer sr-only" required>
                            <div class="w-5 h-5 border-2 border-surface-300 rounded-lg peer-focus:ring-4 peer-focus:ring-primary-500/20 peer-checked:bg-primary-600 peer-checked:border-primary-600 transition-all flex items-center justify-center">
                                <svg class="w-3 h-3 text-white scale-0 peer-checked:scale-100 transition-transform" fill="none" viewBox="0 0 24 24" stroke-width="4" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                                </svg>
                            </div>
                            <span class="mr-3 text-xs font-bold text-surface-600 select-none group-hover:text-surface-900 transition-colors">
                                أوافق على 
                                <a target="_blank" href="{{ route('terms.show') }}" class="text-primary-600 hover:text-primary-500 transition-colors underline">شروط الخدمة</a> 
                                و 
                                <a target="_blank" href="{{ route('policy.show') }}" class="text-primary-600 hover:text-primary-500 transition-colors underline">سياسة الخصوصية</a>
                            </span>
                        </label>
                    </div>
                @endif
            ...
            <div class="mt-8 pt-8 border-t border-surface-200 text-center text-sm font-bold text-surface-600">
                لديك حساب بالفعل؟ 
                <a href="{{ route('login') }}" class="text-primary-600 hover:text-primary-500 transition-colors mr-1">تسجيل الدخول</a>
            </div>
        </div>
    </div>
</x-guest-layout>
