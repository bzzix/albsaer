    ...
    <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-[480px] reveal-anim">
        ...
        <div class="glass-panel overflow-hidden rounded-3xl p-8 sm:p-12 shadow-glass relative">
            ...
            <div class="mb-8 text-sm leading-relaxed text-surface-600 text-right font-bold relative z-10">
                هذه منطقة آمنة من التطبيق. يرجى تأكيد كلمة المرور الخاصة بك قبل المتابعة لمواصلة العمليات الحساسة.
            </div>
            ...
            @if ($errors->any())
                <div class="mb-6 rounded-2xl border border-red-200 bg-red-50/50 p-4 backdrop-blur-sm relative z-20">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="font-bold text-red-800 text-sm">كلمة المرور غير صحيحة</h3>
                    </div>
                </div>
            @endif

            <form method="POST" action="{{ route('password.confirm') }}" class="space-y-6 relative z-10" x-data="{ showPw: false }">
                @csrf
                ...
                <div class="space-y-2 group">
                    ...
                    <div class="relative">
                        ...
                        <input id="password" name="password" :type="showPw ? 'text' : 'password'" autocomplete="current-password" required autofocus
