<div x-data="{ 
    open: false
     ...
     wire:ignore.self
     @open-modal.window="
         if ($event.detail[0] === 'academic-year-form') { 
             open = true; 
         }
     " 
     @close-modal.window="
            ...
            <form @submit.prevent="$wire.save()">
                ...
                <div class="px-6 py-4 border-b border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-800 flex justify-between items-center">
                    <h3 class="text-xl font-bold text-surface-900 dark:text-surface-50">
                        {{ $isEditMode ? 'تعديل العام الأكاديمي' : 'إضافة عام أكاديمي جديد' }}
                    </h3>
                ...
                <div class="p-6 space-y-6 max-h-[calc(100vh-200px)] overflow-y-auto">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-1">
                            <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-1">اسم العام الأكاديمي <span class="text-red-500">*</span></label>
                            <input type="text" wire:model.blur="name" class="premium-input w-full" placeholder="مثال: 2025-2026">
                            @error('name') <span class="text-sm text-red-500 mt-1 block">{{ $message }}</span> @enderror
                        </div>

                        <!-- Code -->
                        <div class="md:col-span-1">
                            <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-1">رمز العام (Code) <span class="text-red-500">*</span></label>
                            <input type="text" wire:model.blur="code" class="premium-input w-full" placeholder="مثال: Y-25-26">
                        ...
                        <div class="md:col-span-1">
                            ...
                            <input type="date" wire:model.blur="start_date" class="premium-input w-full text-left" dir="ltr">
                        ...
                        <div class="md:col-span-1">
                            ...
                            <input type="date" wire:model.blur="end_date" class="premium-input w-full text-left" dir="ltr">
                        ...
                        <div class="md:col-span-2 flex items-center justify-between p-4 bg-surface-100 dark:bg-surface-700/50 rounded-xl border border-surface-200 dark:border-surface-600">
                            <div>
                                <h4 class="text-sm font-bold text-surface-900 dark:text-surface-100">نشاط العام الأكاديمي</h4>
                                <p class="text-xs text-surface-500 dark:text-surface-400 mt-1">تحديد ما إذا كان هذا العام الأكاديمي متاحاً للاستخدام حالياً.</p>
                            </div>
                ...
                <div class="px-6 py-4 bg-surface-50 dark:bg-surface-800/50 border-t border-surface-200 dark:border-surface-700 flex items-center justify-end gap-3">
                    ...
                    <button type="submit" class="px-6 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-xl font-bold shadow-sm shadow-primary-500/20 transition-all flex items-center justify-center gap-2 min-w-[120px]">
                        <span wire:loading.remove wire:target="save">
                            {{ $isEditMode ? 'حفظ التعديلات' : 'إضافة العام' }}
                        </span>
